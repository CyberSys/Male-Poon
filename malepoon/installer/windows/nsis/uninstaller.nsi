...
Section "Uninstall"
  ...
  ${un.CleanUpdateDirectories} "Mozilla\MalePoon" "Mozilla\updates"
  ...
  ${un.RegCleanAppHandler} "MalePoonURL"
  ${un.RegCleanAppHandler} "MalePoonHTML"
  ${un.RegCleanProtocolHandler} "ftp"
  ${un.RegCleanProtocolHandler} "http"
  ${un.RegCleanProtocolHandler} "https"

  ClearErrors
  ReadRegStr $R9 HKCR "MalePoonHTML" ""
  ; Don't clean up the file handlers if the MalePoonHTML key still exists since
  ; there should be a second installation that may be the default file handler
  ${If} ${Errors}
    ${un.RegCleanFileHandler}  ".htm"   "MalePoonHTML"
    ${un.RegCleanFileHandler}  ".html"  "MalePoonHTML"
    ${un.RegCleanFileHandler}  ".shtml" "MalePoonHTML"
    ${un.RegCleanFileHandler}  ".xht"   "MalePoonHTML"
    ${un.RegCleanFileHandler}  ".xhtml" "MalePoonHTML"
    ${un.RegCleanFileHandler}  ".oga"  "MalePoonHTML"
    ${un.RegCleanFileHandler}  ".ogg"  "MalePoonHTML"
    ${un.RegCleanFileHandler}  ".ogv"  "MalePoonHTML"
    ${un.RegCleanFileHandler}  ".webm"  "MalePoonHTML"
  ${EndIf}
  ...
  ; installing even if there is another install of MalePoon that is set as the
  ...
  ${If} $R9 == "false"
    ...
    DeleteRegKey HKLM "$0"
    DeleteRegKey HKCU "$0"
    StrCpy $0 "Software\Classes\MIME\Database\Content Type\application/x-xpinstall;app=MalePoon"
    DeleteRegKey HKLM "$0"
    DeleteRegKey HKCU "$0"
  ${Else}
    ReadRegStr $R1 HKLM "$0" ""
  ...
  ; If MalePoon.exe was successfully deleted yet we still need to restart to
  ; remove other files create a dummy MalePoon.exe.moz-delete to prevent the
  ...
  ; Users who uninstall then reinstall expecting MalePoon to use a clean profile
  ; may be surprised during first-run. This key is checked during startup of MalePoon and
  ; subsequently deleted after checking. If the value is found during startup
  ; the browser will offer to Reset MalePoon. We use the UpdateChannel to match
  ; uninstalls of MalePoon-release with reinstalls of MalePoon-release, for example.
  WriteRegStr HKCU "Software\Mozilla\MalePoon" "Uninstalled-${UpdateChannel}" "True"
