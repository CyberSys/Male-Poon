...
!macro PostUpdate
  ...
  ; start menu tile.  In case there are 2 MalePoon installations, we only do
  ; this if the application being updated is the default.
  ReadRegStr $0 HKCU "Software\Microsoft\Windows\Shell\Associations\UrlAssociations\http\UserChoice" "ProgId"
  ${If} $0 == "MalePoonURL"
  ${AndIf} $9 != 0 ; We're not running in session 0
    ReadRegStr $0 HKCU "Software\Classes\MalePoonURL\shell\open\command" ""
...
!macro AddAssociationIfNoneExist FILE_TYPE
  ...
  ${If} ${Errors}
    WriteRegStr SHCTX "SOFTWARE\Classes\${FILE_TYPE}"  "" "MalePoonHTML"
  ${EndIf}
  WriteRegStr SHCTX "SOFTWARE\Classes\${FILE_TYPE}\OpenWithProgids" "MalePoonHTML" ""
!macroend
!define AddAssociationIfNoneExist "!insertmacro AddAssociationIfNoneExist"

; Adds the protocol and file handler registry entries for making MalePoon the
...
!macro SetHandlers
  ...
  ; Associate the file handlers with MalePoonHTML
  ReadRegStr $6 SHCTX "$0\.htm" ""
  ${If} "$6" != "MalePoonHTML"
    WriteRegStr SHCTX "$0\.htm"   "" "MalePoonHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.html" ""
  ${If} "$6" != "MalePoonHTML"
    WriteRegStr SHCTX "$0\.html"  "" "MalePoonHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.shtml" ""
  ${If} "$6" != "MalePoonHTML"
    WriteRegStr SHCTX "$0\.shtml" "" "MalePoonHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xht" ""
  ${If} "$6" != "MalePoonHTML"
    WriteRegStr SHCTX "$0\.xht"   "" "MalePoonHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xhtml" ""
  ${If} "$6" != "MalePoonHTML"
    WriteRegStr SHCTX "$0\.xhtml" "" "MalePoonHTML"
  ${EndIf}
  ...
  ; An empty string is used for the 5th param because MalePoonHTML is not a
  ; protocol handler
  ${AddDisabledDDEHandlerValues} "MalePoonHTML" "$2" "$8,1" \
                                 "${AppRegName} HTML Document" ""

  ${AddDisabledDDEHandlerValues} "MalePoonURL" "$2" "$8,1" "${AppRegName} URL" \
                                 "true"
...
; Adds the HKLM\Software\Clients\StartMenuInternet\MALEPOON.EXE registry
...
!macro SetStartMenuInternet RegKey
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".htm"   "MalePoonHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".html"  "MalePoonHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".shtml" "MalePoonHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xht"   "MalePoonHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xhtml" "MalePoonHTML"

  WriteRegStr ${RegKey} "$0\Capabilities\StartMenu" "StartMenuInternet" "$R9"

  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "ftp"    "MalePoonURL"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "http"   "MalePoonURL"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "https"  "MalePoonURL"
...
; The IconHandler reference for MalePoonHTML can end up in an inconsistent state
; due to changes not being detected by the IconHandler for side by side
; installs (see bug 268512). The symptoms can be either an incorrect icon or no
; icon being displayed for files associated with MalePoon (does not use SHCTX).
!macro FixShellIconHandler RegKey
  ClearErrors
  ReadRegStr $1 ${RegKey} "Software\Classes\MalePoonHTML\ShellEx\IconHandler" ""
  ${Unless} ${Errors}
    ReadRegStr $1 ${RegKey} "Software\Classes\MalePoonHTML\DefaultIcon" ""
    ${GetLongPath} "$INSTDIR\${FileMainEXE}" $2
    ${If} "$1" != "$2,1"
      WriteRegStr ${RegKey} "Software\Classes\MalePoonHTML\DefaultIcon" "" "$2,1"
    ${EndIf}
  ${EndUnless}
!macroend
...
!macro SetUninstallKeys
  ...
  ${If} $2 == ""
    ...
    ${WriteRegStr2} $1 "$0" "Publisher" "Manchild Productions" 0
...
; to MalePoonHTML when there is a value for PersistentHandler or by removing the
...
!macro FixBadFileAssociation FILE_TYPE
  ...
  ${If} "$2" != ""
    ; Since there is a persistent handler remove MalePoonHTML as the default
    ; value from both HKCU and HKLM if it set to MalePoonHTML.
    ${If} "$0" == "MalePoonHTML"
    ...
    ${EndIf}
  ${ElseIf} "$0" == "MalePoonHTML"
    ; Since KHCU is set to MalePoonHTML remove MalePoonHTML as the default value
...
!macro UpdateProtocolHandlers
  ...
  ${IsHandlerForInstallDir} "MalePoonHTML" $R9
  ${If} "$R9" == "true"
    ; An empty string is used for the 5th param because MalePoonHTML is not a
    ; protocol handler.
    ${AddDisabledDDEHandlerValues} "MalePoonHTML" "$2" "$8,1" \
  ...
  ${IsHandlerForInstallDir} "MalePoonURL" $R9
  ${If} "$R9" == "true"
    ${AddDisabledDDEHandlerValues} "MalePoonURL" "$2" "$8,1" \
                                   "${AppRegName} URL" "true"
  ${EndIf}
...
!macro RemoveDeprecatedKeys
  ...
  ; update if the DefaultIcon is from malepoon.exe.
  ${RegCleanAppHandler} "gopher"

  ; Remove support for launching chrome urls from the shell during install or
  ; update if the DefaultIcon is from malepoon.exe (Bug 301073).
  ${RegCleanAppHandler} "chrome"

  ; Remove protocol handler registry keys added by the MS shim
  DeleteRegKey HKLM "Software\Classes\MalePoon.URL"
  DeleteRegKey HKCU "Software\Classes\MalePoon.URL"
  ...
  ; Delete gopher from the user's UrlAssociations if it points to MalePoonURL.
  ...
  ${If} ${FileExists} "$0"
    ...
    ${If} ${Errors}
      ...
      ${If} ${AtLeastWin7}
        ...
        ${If} ${AtMostWin2008R2}
          ; Check if the Male Poon is the http handler for this user
