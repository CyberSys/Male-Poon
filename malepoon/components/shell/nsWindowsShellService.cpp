...
//   are mapped like so:
//
//   HKCU\SOFTWARE\Classes\.<ext>\      (default)         REG_SZ     MalePoonHTML
//
//   as aliases to the class:
//
//   HKCU\SOFTWARE\Classes\MalePoonHTML\
...
//
//   HKCU\SOFTWARE\Classes\MalePoonURL\  (default)         REG_SZ     <appname> URL
...
//   -------------------------------------------------
//   The following keys are set to make MalePoon appear in the Start Menu as the
//   browser:
//   
//   HKCU\SOFTWARE\Clients\StartMenuInternet\MalePoon.EXE\
...
#define APP_REG_NAME L"Male Poon"
...
// MalePoon is the default browser for file handlers since other applications
...
// handler to another application and still have MalePoon check if it is the
...
static SETTING gSettings[] = {
  ...
  { MAKE_KEY_NAME1("MalePoonHTML", SOC), VAL_OPEN, OLD_VAL_OPEN },

  // Protocol Handler Class - for Vista and above
  { MAKE_KEY_NAME1("MalePoonURL", SOC), VAL_OPEN, OLD_VAL_OPEN },
...
// since they are only checked when MalePoon is the default browser and if they
// are incorrect they are fixed without notifying the user.
static SETTING gDDESettings[] = {
  // File Handler Class
  { MAKE_KEY_NAME1("Software\\Classes\\MalePoonHTML", SOD) },

  // Protocol Handler Class - for Vista and above
  { MAKE_KEY_NAME1("Software\\Classes\\MalePoonURL", SOD) },
             ...
             LPCWSTR aClassName)
{
  ...
  LPCWSTR progID = isProtocol ? L"MalePoonURL" : L"MalePoonHTML";
 ...
 * This only checks for MalePoonURL and if aCheckAllTypes is set, then
 * it also checks for MalePoonHTML.  Note that those ProgIDs are shared
 * by all MalePoon browsers.
*/
bool
                                        ...
                                        bool* aIsDefaultBrowser)
{
  ...
  *aIsDefaultBrowser = true;
  ...
  // that was used to launch MalePoon which is not necessarily a long path.
  ...
  for (; settings < end; ++settings) {
    ...
    if (REG_FAILED(res) ||
      ...
      if (REG_FAILED(res)) {
        ...
        // application when setting MalePoon as the default browser.
        *aIsDefaultBrowser = false;
        return NS_OK;
      }
  ...
  // Only check if MalePoon is the default browser on Vista and above if the
  // previous checks show that MalePoon is the default browser.
  if (*aIsDefaultBrowser) {
  ...
  // account didn't perform a MalePoon update this will check if MalePoon is the
  // default browser and if dde is disabled for each handler
  // and if it isn't disable it. When MalePoon is not the default browser the
  ...
  if (*aIsDefaultBrowser && aForAllTypes) {
    ...
    // update it using the helper application when setting MalePoon as the
    // default browser.
    if (REG_FAILED(res)) {
      *aIsDefaultBrowser = false;
    }
                                            ...
                                            int32_t aPosition)
{
  ...
  // eventually, the path is "%APPDATA%\Mozilla\MalePoon\Desktop Background.bmp"
